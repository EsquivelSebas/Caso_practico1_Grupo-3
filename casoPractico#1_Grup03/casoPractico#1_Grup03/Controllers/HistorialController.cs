...
namespace casoPractico_1_Grup03.Controllers
{
    ...
    public class HistorialController : Controller
    {
            ...
            public async Task<ActionResult<Historial>> GetHistorialByID(int id)
            {
                ...
                return Ok(new { user = historial, mensaje });
            }
            ...
            //post Users (ADD)
            [HttpPost]
            public async Task<ActionResult<Historial>> AddUsers([FromBody] Historial historial)
            {
                ...
                return CreatedAtAction(nameof(GetHistorialByID), new { id = historial.ID }, new { user = historial, mensajeCorrecto });
            ...
            [HttpPut("{id}")]
            public async Task<IActionResult> UpdateUserById(int id, [FromBody] Historial historial)
            {
                ...
                var existingUser = await _context.G3Historial.FindAsync(id);
                ...
                _context.Entry(existingUser).CurrentValues.SetValues(historial);
                ...
                {
                    if (!_context.G3Historial.Any(u => u.ID == id))
                    {
                        return NotFound("The user does not exist.");
                    }
                    else
            ...
            [HttpDelete("{id}")]
            public async Task<ActionResult<Historial>> deleteUsersByID(int id)
            {
                //validation if exists
                var users = await _context.G3Historial.FindAsync(id);
                if (users == null)
                {
                    var mensajeError = new { msg = "User not Found.....😭" };
                    return NotFound(mensajeError);
                }//but if it exists
                _context.G3Historial.Remove(users);
                await _context.SaveChangesAsync();
